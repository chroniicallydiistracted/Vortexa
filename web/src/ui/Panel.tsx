...
export default function Panel() {
  ...
  const tileBase =
    (import.meta as any).env?.VITE_TILE_BASE || "http://localhost:4000/tiles";
  return (
    <div style={{ padding: 12, overflow: "auto" }}>
      <h3>Time & Playback</h3>
      <div style={{ marginBottom: 16 }}>
        <label>
          <div style={{ fontSize: 12, opacity: 0.7 }}>Time (YYYY-MM-DD)</div>
          <input
            type="date"
            value={time}
        ...
        <div style={{ marginTop: 8 }}>
          <button onClick={() => togglePlaying()}>
            {playing ? "Pause" : "Play"} (daily)
          </button>
          <button
            onClick={() => setTime(new Date().toISOString().slice(0, 10))}
            style={{ marginLeft: 8 }}
          >
            Today
          </button>
      ...
      <div style={{ marginBottom: 12 }}>
        <button
          onClick={async () => {
            try {
              await ensureGibsTimestamps("GOES-East_ABI_GeoColor");
            } catch {
              /* noop */
            }
          }}
        ...
        <button
          disabled={gibsTimestamps.length < 2 || !!gibsLoadError}
          style={{ marginLeft: 8 }}
        ...
        <div style={{ marginTop: 6, fontSize: 12 }}>
          {gibsLoadError ? (
            <span style={{ color: "crimson" }}>Load failed</span>
          ) : gibsSelectedTime ? (
            `Selected: ${gibsSelectedTime}`
          ) : gibsTimestamps.length ? (
            "Loaded timestamps"
          ) : (
            ""
          )}
        ...
        <div style={{ marginTop: 6 }}>
          <label style={{ fontSize: 12 }}>
            ...
            <input
              ...
              max={8}
              value={gibsFps}
              onChange={(e) => setGibsFps(Number(e.currentTarget.value || 4))}
              style={{ width: 60 }}
            />
          </label>
        </div>
        {gibsTimestamps.length > 0 && (
          ...
          >
            {gibsTimestamps.slice(-50).map((t: string) => (
              <div
                key={t}
                style={{
                  cursor: "pointer",
                  color: t === gibsSelectedTime ? "#1976d2" : "#333",
                }}
                onClick={() => setGibsSelectedTime(t)}
              >
                {t}
              </div>
            ))}
        ...
        {LAYER_PRESETS.map((p) => (
          <button
            ...
            onClick={() => {
              ...
              const realized = p.layers.map((l) => ({
                ...l,
                templateRaw: l.templateRaw.replace("{TILE_BASE}", tileBase),
              }));
      ...
      <div style={{ marginBottom: 12 }}>
        ...
        >
          Add Demo: NASA GIBS GeoColor
        </button>
      </div>
      {!data
        ? "Loading..."
        : data.entries.slice(0, 20).map((e, i) => {
            const id = `${e.provider}-${e.layer}`
              .toLowerCase()
              .replace(/[^a-z0-9]+/g, "-");
            return (
              <div
              ...
              >
                <div>
                  <strong>{e.layer}</strong> <small>({e.provider})</small>
                </div>
                <div>
                  <small>
                    {e.access} • {e.coverage}
      ...
      {layers.length === 0 ? (
        <div style={{ opacity: 0.6 }}>No layers</div>
      ) : (
        layers.map((l: any) => (
          <div
            key={l.id}
          ...
          >
            ...
            <input
              ...
              step={0.05}
              value={l.opacity ?? 1}
              onChange={(e) => setOpacity(l.id, Number(e.currentTarget.value))}
            />
        ...
        <button
          onClick={() => {
            const link =
              location.origin +
              location.pathname +
              buildHash({ time, view: useStore.getState().view, layers });
