...
export function CesiumGlobe(){
  ...
  const { Viewer, EllipsoidTerrainProvider, Cartesian3, UrlTemplateImageryProvider, WebMapTileServiceImageryProvider } = Cesium as any;
        ...
        try {
          const provider = new UrlTemplateImageryProvider({ url: baseTemplate, credit: 'Tiles © CartoDB, Data © OpenStreetMap contributors', minimumLevel:0, maximumLevel: 18 });
        ...
        viewer.camera.changed.addEventListener(()=>{
          ...
          setView({ lat: c.latitude * 180/Math.PI, lon: c.longitude * 180/Math.PI });
        });
  ...
  useEffect(()=>{
    (async () => {
      if(!ready) return;
      const Cesium = await import('cesium');
      const viewer = (Cesium as any).Viewer?.instances?.[0] || (document.querySelector('canvas.cesium-widget') as any)?._cesiumWidget?.viewer; // fallback heuristic
      if(!viewer) return;
      const layers = viewer.imageryLayers;
      const existing = layers._layers.find((l:any)=> l._credit && /GOES-East GeoColor/i.test(l._credit?.html || ''));
      if(gibsOn){
        // Determine time parameter (selected or latest available)
        const timeIso = (gibsSelectedTime || gibsTimestamps[gibsTimestamps.length-1] || new Date().toISOString().slice(0,19)+'Z');
        const template = ((import.meta as any).env?.VITE_GIBS_WMTS_TILE_URL) || '/api/gibs/geocolor/{z}/{x}/{y}.jpg?time={time}';
        if(!existing){
          const url = template.replace('{TileMatrix}','{z}').replace('{TileRow}','{y}').replace('{TileCol}','{x}').replace('{time}', encodeURIComponent(timeIso));
          const provider = new (Cesium as any).UrlTemplateImageryProvider({ url, credit: 'GOES-East GeoColor (NASA GIBS)' });
  ...
  useEffect(()=>{
    (async () => {
      if(!ready) return;
      const Cesium = await import('cesium');
      const viewer = (Cesium as any).Viewer?.instances?.[0] || (document.querySelector('canvas.cesium-widget') as any)?._cesiumWidget?.viewer;
      if(!viewer) return;
      // Remove existing collection if toggled off
      if(!showFirms){
        const existing = (viewer.scene.primitives as any)._primitives.find((p:any)=> p._westfamTag==='firms');
        if(existing) viewer.scene.primitives.remove(existing);
        return;
      }
      // If already present do nothing
      const exists = (viewer.scene.primitives as any)._primitives.find((p:any)=> p._westfamTag==='firms');
      if(exists) return;
      try {
  ...
  useEffect(()=>{
    (async () => {
      ...
      if(!viewer) return;
      const layers = viewer.imageryLayers;
      const existing = layers._layers.find((l:any)=> l._credit && /OWM Temperature/i.test(l._credit?.html || ''));
      if(!showOwmTemp && existing){ layers.remove(existing, true); return; }
      if(showOwmTemp && !existing){
        ...
        const provider = new (Cesium as any).UrlTemplateImageryProvider({ url: template, credit: 'OWM Temperature (OpenWeatherMap)' });
        layers.addImageryProvider(provider, 2);
      }
    })();
  }, [showOwmTemp, ready]);

  return <div ref={containerRef} style={{position:'absolute', inset:0, background:'#000', visibility: ready? 'visible':'hidden'}} />;
}
