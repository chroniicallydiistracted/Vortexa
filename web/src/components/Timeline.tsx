...
export default function Timeline({
  ...
  const value = Math.max(
    0,
    Math.min(sliderMax, Math.round((currentTime - baseStart) / hourMs)),
  );
  ...
  const label =
    new Date(currentTime).toISOString().replace("T", " ").substring(0, 16) +
    "Z";
  return (
    <div style={containerStyle}>
      <button style={btnStyle} onClick={onToggle}>
        {playing ? "Pause" : "Play"}
      </button>
      <input
        type="range"
