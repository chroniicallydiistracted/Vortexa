...
describe('TimeBar drag behavior (event bridge)', () => {
    ...
    it('emits multiple temp changes but single commit on end', () => {
        const temps = [];
        const commits = [];
        const onTempChange = vi.fn((v) => temps.push(v));
        const onCommit = vi.fn((v) => commits.push(v));
        const baseStart = Date.UTC(2025, 7, 24, 0, 0, 0);
        const hoursSpan = 12;
        let currentTime = baseStart;
        let hourValue = 0;
        const setCurrentTime = vi.fn((t) => {
            currentTime = t;
        });
        const setHourValue = vi.fn((h) => {
            hourValue = h;
        });
        renderWithMantine(_jsx(TimeBar, { playing: false, togglePlay: () => { }, baseStart: baseStart, hoursSpan: hoursSpan, currentTime: currentTime, setCurrentTime: setCurrentTime, currentDate: new Date(baseStart), setCurrentDate: () => { }, hourValue: hourValue, setHourValue: setHourValue, speed: '1x', setSpeed: () => { }, onTempChange: onTempChange, onCommit: onCommit, testEventBridge: true }));
