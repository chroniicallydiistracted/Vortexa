...
export default function Map({ activeLayerSlug, catalog, onMapReady, currentTime }) {
    ...
    useEffect(() => {
        ...
        const baseTemplate = import.meta.env.VITE_BASEMAP_TILE_URL || '/api/cartodb/positron/{z}/{x}/{y}.png';
        const style = {
            version: 8,
            sources: {
                basemap: {
        ...
        return () => {
            try {
                ...
                console.debug('Map: cleanup failed', e.message);
            }
            try {
                map.remove();
            }
    ...
    useEffect(() => {
        ...
        const entries = Array.isArray(catalog)
            ? catalog
            : catalog && Array.isArray(catalog.layers)
                ? catalog.layers
                : [];
        ...
        if (entry.source_type === 'raster' && entry.tile_url_template) {
            ...
            fetch(entry.api_endpoint)
                .then(async (r) => {
                if (!r.ok)
                    throw new Error(`fetch ${entry.api_endpoint} ${r.status}`);
                ...
                if (!stillValid())
                    return;
                if (map.getSource('active-vector'))
                    return;
    ...
    useEffect(() => {
        ...
        fetch('/api/alerts')
            ...
            if (!fc?.features)
                return;
            if (map.getSource('alerts-static'))
                return;
