...
export function encodeLayers(layers: Layer[]): string | undefined {
  ...
  return layers.map(l=> l.opacity!=null? `${l.id}:${Number(l.opacity).toFixed(2)}` : l.id).join(',');
}
...
export function buildHash(state: { time: string; view: { lat:number; lon:number; zoom:number }; layers: Layer[] }): string {
  const obj: PermalinkState = { t: state.time, z: Number(state.view.zoom.toFixed(2)), lat: Number(state.view.lat.toFixed(3)), lon: Number(state.view.lon.toFixed(3)) };
