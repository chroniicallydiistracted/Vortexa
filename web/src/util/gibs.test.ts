...
describe('gibs buildTileUrl', () => {
  ...
  it('prefetch uses HEAD and correct extension', async () => {
    const spy = vi.spyOn(globalThis, 'fetch').mockResolvedValue({ ok:true, headers: new Headers() } as any);
  ...
  it('fetchTimestamps returns [] on non-json content-type', async () => {
    const spy = vi.spyOn(globalThis, 'fetch').mockResolvedValue({ ok:true, headers: { get: ()=> 'text/html' }, json: async ()=> ({ timestamps:['x']}) } as any);
