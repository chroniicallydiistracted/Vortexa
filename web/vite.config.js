...
const backend = process.env.VITE_BACKEND_URL || 'http://127.0.0.1:4000'; // configurable in dev
    ...
    return {
        ...
        transform(code, id) {
            if (id.includes('protobufjs/dist/minimal/protobuf.js')) {
                const before = code;
                const replaced = code.replace('var mod = eval("quire".replace(/^/,"re"))(moduleName); // eslint-disable-line no-eval', 'var mod = null; // stripped eval (dynamic require) for bundler safety');
                if (before !== replaced) {
