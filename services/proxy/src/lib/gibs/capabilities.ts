...
const DEFAULT_CAPS_URL = process.env.GIBS_CAPS_URL || 'https://gibs.earthdata.nasa.gov/wmts/epsg3857/best/wmts.cgi?SERVICE=WMTS&REQUEST=GetCapabilities';
const DEFAULT_TILE_BASE = process.env.GIBS_TILE_BASE || 'https://gibs.earthdata.nasa.gov/wmts/epsg3857/best';
...
export async function getTimestamps(layerId: string): Promise<string[]> {
  ...
  const layerRegex = new RegExp(`<Layer>[\\s\\S]*?<(?:(?:ows:)?Title)>\\s*${escaped}\\s*<\\/(?:ows:)?Title>[\\s\\S]*?<\\/Layer>`, 'i');
  const layerBlockMatch = xml.match(layerRegex);
  if (!layerBlockMatch) { tsCache.set(layerId, []); return []; }
  ...
  const dimMatch = layerBlock.match(/<Dimension[^>]*name=\"time\"[^>]*>([\s\S]*?)<\/Dimension>/i)
    || layerBlock.match(/<Dimension[\s\S]*?(?:name=\"time\"|<Identifier>time<\/Identifier>)[\s\S]*?<Value>([\s\S]*?)<\/Value>/i);
  if (!dimMatch) { tsCache.set(layerId, []); return []; }
...
export function pickTms(layerId: string): 'GoogleMapsCompatible_Level8' | 'GoogleMapsCompatible_Level9' {
...
export interface BuildTileUrlOpts {
  layerId: string; z: number; y: number; x: number; time: string; tms?: string; ext?: string;
}
