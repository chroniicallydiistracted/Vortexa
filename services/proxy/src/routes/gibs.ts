...
gibsRouter.get("/timestamps", async (req, res) => {
  try {
    ...
    if (!layer)
      return res.status(400).json({ error: "missing layer parameter" });
  ...
  try {
    ...
    if (explicitTime) {
      ...
      if (!ts.includes(explicitTime)) {
        return res
          .status(400)
          .json({ error: "invalid time for layer", layer, time: explicitTime });
      }
...
gibsRouter.get("/geocolor/:z/:x/:y.:ext", (req, res) => {
  ...
  res.redirect(
    302,
    `/api/gibs/tile/GOES-East_ABI_GeoColor/${z}/${y}/${x}.${ext}`,
  );
});
