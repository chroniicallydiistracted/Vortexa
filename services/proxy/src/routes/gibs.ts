...
import { getTimestamps, getLatestTimestamp, buildTileUrl, pickTms } from '../lib/gibs/capabilities.js';
...
gibsRouter.get('/timestamps', async (req, res) => {
  try {
    ...
    if (/gibs_capabilities_fetch_failed/.test(msg)) return res.status(502).json({ error: 'upstream_capabilities_failed' });
...
gibsRouter.get('/tile/:layer/:z/:y/:x.:ext?', async (req, res) => {
  try {
    ...
    if (!upstream.ok) return res.status(upstream.status).json({ error: 'upstream_error', status: upstream.status });
    const buf = Buffer.from(await upstream.arrayBuffer());
    res.set('Content-Type', upstream.headers.get('content-type') || (ext === 'jpg' ? 'image/jpeg' : 'image/png'));
    res.set('Cache-Control', 'public, max-age=60');
    return res.send(buf);
  } catch (e: any) {
