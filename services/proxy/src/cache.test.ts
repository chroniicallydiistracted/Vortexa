...
class FakeBody { constructor(private data: Buffer){} pipe(res:any){ res.end(this.data); } }
...
class FakeS3 extends S3Client {
  ...
  send(cmd: any){
    if (cmd instanceof GetObjectCommand) {
      ...
      return Promise.resolve({ Body: new FakeBody(stored.body), ContentType: stored.contentType });
    }
    if (cmd instanceof PutObjectCommand) {
  stored = { key: cmd.input.Key as string, body: cmd.input.Body as Buffer, contentType: cmd.input.ContentType };
...
describe('proxy cache flow', () => {
  const app = createApp({ allowHosts: ['example.com'], s3Bucket: 'cache-bkt', s3Client: new FakeS3() as any });
