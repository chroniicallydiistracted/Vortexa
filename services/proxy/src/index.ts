...
export function createApp(opts: CreateAppOptions = {}) {
  ...
  interface DeepHealthPayload {
    ...
    components: {
      ...
      rainviewer: DeepHealthComponent<{
        frames: number;
        past: number;
        nowcast: number;
        age_ms: number;
      ...
      dynamodb_alerts: DeepHealthComponent<{
        table: string;
        itemCount?: number;
      }>;
    };
  ...
  async function runDeepHealth(): Promise<DeepHealthPayload> {
    ...
    // Rainviewer meta
    let rainviewer: DeepHealthComponent<{
      frames: number;
      past: number;
      nowcast: number;
      age_ms: number;
    ...
    try {
      ...
      rainviewer = {
        ...
        data: {
          frames,
          past: meta.past.length,
          nowcast: meta.nowcast.length,
    ...
    const ok =
      Object.values(upstream_hosts).every((c) => c.ok) &&
      rainviewer.ok &&
      gibs_goes_b13.ok &&
      dynamodb_alerts.ok;
    return {
  ...
  function gibsTake(ip: string): boolean {
    ...
    if (!b) {
      b = { tokens: gibsBurst, last: now };
      gibsBuckets.set(ip, b);
    }
    const dt = (now - b.last) / 1000;
    if (dt > 0) {
      b.tokens = Math.min(gibsBurst, b.tokens + dt * gibsRefill);
      b.last = now;
    }
    if (b.tokens >= 1) {
      b.tokens -= 1;
  ...
  app.get('/api/radar/tiles/:z/:x/:y.png', async (req, res) => {
    try {
      ...
      const ordered = [
        ...meta.nowcast.slice().sort((a, b) => a.time - b.time),
        ...meta.past.slice().sort((a, b) => a.time - b.time),
      ];
  ...
  let goesB13Meta: { ts: number; latest: string | null } = {
    ts: 0,
    latest: null,
  };
  async function getGoesB13Timestamp(timeoutMs = 5000): Promise<string | null> {
    ...
    try {
      ...
      const dimMatch = slice.match(
        /<Dimension[^>]*>[^<]*<Identifier>time<\/Identifier>[\s\S]*?<Default>([^<]+)<\/Default>/i,
      );
