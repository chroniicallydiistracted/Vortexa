...
describe('gibs ops polish (rate limiting + metrics)', () => {
  it('enforces rate limiting returning 429 after burst', async () => {
    ...
    (global as any).__TEST_FETCH__ = vi.fn()
      ...
      .mockResolvedValue({ ok:true, status:200, text: async ()=> `<?xml version=\"1.0\"?><Capabilities><Contents><Layer><Title>GOES-East_ABI_GeoColor</Title><Dimension name=\"time\">2025-08-24T18:00:00Z 2025-08-24T19:00:00Z</Dimension></Layer></Contents></Capabilities>` })
      // tile fetches (second call onward)
      .mockResolvedValue({ ok:true, status:200, headers: new Map([["content-type","image/png"]]), arrayBuffer: async ()=> new Uint8Array([1]).buffer });
    const app = createApp();
