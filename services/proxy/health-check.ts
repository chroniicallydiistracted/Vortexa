...
ensureEnv('NWS_USER_AGENT', process.env.NWS_USER_AGENT || 'WestFamDev/0.1 (contact: dev@example.com)');
...
if (!QUIET) {
  ...
  console.log('[health-check] Time budgets:', { MAX_LAYER_MS, MAX_TOTAL_MS, RAINVIEWER_MAX_ATTEMPTS });
}
...
interface Catalog { layers: Layer[] }
...
for (const p of catalogPaths) { if (fs.existsSync(p)) { catalogFile = p; break; } }
if (!catalogFile) {
...
interface Result { slug: string; status: number | null; ok: boolean; message: string; };
const results: Result[] = [];

// --- Managed proxy helpers -------------------------------------------------
interface ManagedProxy { proc: ReturnType<typeof spawn> | null; started: boolean }
...
async function forceKillPort(port: number) {
  try {
    const pids = execSync(`lsof -t -iTCP:${port} -sTCP:LISTEN || true`, { stdio: ['ignore','pipe','ignore'] }).toString().split(/\s+/).filter(Boolean);
    if (pids.length) {
...
async function ensureManagedProxy() {
  ...
  if (!manage) return;
  const needsProxy = catalog.layers.some(l => (l.tile_url_template || l.api_endpoint || '').startsWith('/'));
  ...
  if (!(await portInUse(4000))) {
    ...
    managed.proc.stdout?.on('data', d => { const line = d.toString(); if (/proxy up/.test(line)) console.log('[health-check] proxy up'); });
    managed.proc.stderr?.on('data', d => { const line = d.toString(); if (line.toLowerCase().includes('error')) process.stderr.write('[proxy] '+line); });
    // wait for port open (max 6s)
    for (let i=0;i<30;i++) { if (await portInUse(4000)) { await delay(50); return; } await delay(200); }
    console.warn('[health-check] WARNING: proxy failed to start within timeout');
  } else {
    console.log('[health-check] Port 4000 already in use after kill attempt; proceeding (assume external proxy).');
  }
}
...
async function cleanupManagedProxy() {
  if (managed.started && managed.proc) {
    try {
      ...
      for (let i=0;i<10;i++) { if (managed.proc.exitCode != null) break; await delay(100); }
...
async function fetchWithTimeout(resource: string, opts: { timeout?: number; headers?: any; method?: string } = {}) {
...
async function getRainviewerTimestamps(): Promise<number[]> {
  ...
  try {
    const r = await fetchWithTimeout('https://api.rainviewer.com/public/weather-maps.json', { timeout: 4000 });
    if (r.ok) {
...
async function resolveRainviewer(url: string): Promise<string> {
  ...
  for (const ts of candidates) {
    for (const z of zooms) {
      ...
      try {
        const r = await fetchWithTimeout(candidateBase, { timeout: RAINVIEWER_FETCH_TIMEOUT_MS });
...
async function getGibsLatestTime(): Promise<string | null> {
  ...
  const capUrl = 'https://gibs.earthdata.nasa.gov/wmts/epsg3857/best/wmts.cgi?SERVICE=WMTS&REQUEST=GetCapabilities';
  try {
  const r = await fetchWithTimeout(capUrl, { timeout: GIBS_CAP_TIMEOUT_MS });
    if (!r.ok) throw new Error('caps status ' + r.status);
    const xml = await r.text();
    const layerBlockMatch = xml.match(/<Layer>[\s\S]*?<Identifier>GOES-East_Full_Disk_Band_13_ENHANCED<\/Identifier>[\s\S]*?<\/Layer>/);
    ...
    if (!latest) {
      ...
      if (body.includes('/P')) {
        ...
        latest = body.split(',').map(s=> s.trim()).filter(Boolean).pop() || null;
      } else {
        latest = body || null;
      }
    }
...
async function resolveGibsGoEs(url: string): Promise<string> {
  ...
  if (url.includes('{time}')) return url.replace('{time}', 'current').replace(/\/default\/[^/]+\//, '/default/current/');
...
async function buildTestUrl(layer: Layer): Promise<string | null> {
  ...
  base = base.replace('{z}', root ? '0':'1').replace('{x}', root ? '0':'1').replace('{y}', root ? '0':'1');
...
async function probe(url: string, slug: string): Promise<Result> {
  ...
  try {
    ...
    return { slug, status: null, ok: false, message: e.name === 'AbortError' ? 'TIMEOUT' : (e.message || 'ERROR') };
  }
}
...
async function main(){
  ...
  if (missingConfig.length) {
    console.log(`Config notice: missing ${missingConfig.join(', ')} may cause AUTH or TIMEOUT results.`);
  }
  for (const layer of catalog.layers) {
    const layerStart = Date.now();
    const url = await buildTestUrl(layer);
    if (!url) {
      results.push({ slug: layer.slug, status: null, ok: false, message: 'NO_URL' });
      continue;
    }
    const isPlaceholder = /\/tiles\/(gfs|hrrr)\//.test(url);
    if (isPlaceholder) {
      results.push({ slug: layer.slug, status: 404, ok: false, message: 'PLACEHOLDER_SKIPPED' });
      continue;
    }
    let finalUrl = url;
    ...
    try {
      res = await Promise.race([
        ...
        (async ()=> { await delay(MAX_LAYER_MS); return { slug: layer.slug, status: null, ok: false, message: 'LAYER_TIMEOUT' } as Result; })()
      ]);
    } catch (e: any) {
      res = { slug: layer.slug, status: null, ok: false, message: e?.message || 'ERROR' };
    }
    ...
    if (/^temperature$|^wind$|^total-cloud-cover$|^mean-sea-level-pressure$|^precipitation-type$/.test(layer.slug) && res.status === 401 && devPlaceholderKeys.has((OWM_API_KEY||'').split(/[,;]/)[0])) {
  ...
  for (const r of results) {
    let icon = r.ok ? '✅' : (r.message === 'NOT_FOUND' || r.message === 'PLACEHOLDER_SKIPPED' ? '⚠️' : '❌');
