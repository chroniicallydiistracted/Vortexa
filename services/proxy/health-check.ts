...
const RAINVIEWER_MAX_ATTEMPTS = Number(
  process.env.RAINVIEWER_MAX_ATTEMPTS || 9,
); // (timestamps * zooms)
const RAINVIEWER_FETCH_TIMEOUT_MS = Number(
  process.env.RAINVIEWER_FETCH_TIMEOUT_MS || 1800,
);
...
const OWM_API_KEY =
  process.env.OWM_API_KEY || process.env.OPENWEATHERMAP_API_KEY || "";
...
async function forceKillPort(port: number) {
  try {
    ...
    if (pids.length) {
      console.log(
        `[health-check] Killing processes on :${port} -> ${pids.join(",")}`,
      );
...
async function ensureManagedProxy() {
  ...
  if (!(await portInUse(4000))) {
    ...
    console.warn(
      "[health-check] WARNING: proxy failed to start within timeout",
    );
  } else {
    console.log(
...
async function getRainviewerTimestamps(): Promise<number[]> {
  const now = Date.now();
  if (rainviewerCache && now - rainviewerCache.ts < 60_000)
    return rainviewerCache.timestamps; // 1 min cache
  try {
    ...
    if (r.ok) {
      ...
      const nowcast: number[] =
        data?.radar?.nowcast?.map((p: any) => p.time) || [];
...
async function resolveRainviewer(url: string): Promise<string> {
  ...
  for (const ts of candidates) {
    for (const z of zooms) {
      ...
      attempts++;
      const candidateBase = mutateZoomXYZ(url, z, 0, 0).replace(
        "{time}",
        String(ts),
      );
      try {
...
async function getGibsLatestTime(): Promise<string | null> {
  const now = Date.now();
  if (gibsTimeCache && now - gibsTimeCache.ts < 5 * 60_000)
    return gibsTimeCache.value; // 5 min cache
  const capUrl =
  ...
  try {
    ...
    const dimMatch = block.match(
      /<Dimension[^>]*name="time"[^>]*>([\s\S]*?)<\/Dimension>/,
    );
...
async function resolveGibsGoEs(url: string): Promise<string> {
  ...
  if (latest) {
    ...
    if (url.includes("{time}")) {
      return url
        .replace("{time}", latest)
        .replace(/\/default\/[^/]+\//, `/default/${latest}/`);
    }
  ...
  if (url.includes("{time}"))
    return url
      ...
      .replace(/\/default\/[^/]+\//, "/default/current/");
...
async function resolveDynamicTime(layer: Layer, url: string): Promise<string> {
  ...
  // GIBS GOES Band 13
  if (
    /GOES-East_Full_Disk_Band_13_ENHANCED/.test(url) &&
    url.includes("{time}")
  ) {
...
async function buildTestUrl(layer: Layer): Promise<string | null> {
  ...
  const root =
    layer.slug === "radar-reflectivity" ||
    layer.slug === "cloud-top-temperature-height";
  base = base
    .replace("{z}", root ? "0" : "1")
    .replace("{x}", root ? "0" : "1")
    .replace("{y}", root ? "0" : "1");
  ...
  if (base.includes("YOUR_API_KEY")) {
    if (base.includes("OWM"))
      base = base.replace("YOUR_API_KEY", OWM_API_KEY || "MISSING");
...
async function probe(url: string, slug: string): Promise<Result> {
  ...
  try {
    ...
    if (status === 401 || status === 403)
      return { slug, status, ok: false, message: "AUTH" };
    if (status === 404)
      return { slug, status, ok: false, message: "NOT_FOUND" };
...
async function main() {
  ...
  for (const layer of catalog.layers) {
    ...
    if (!QUIET) {
      ...
      console.log(
        `   ↳ ${layer.slug} finished in ${elapsed}ms (${res.message})`,
      );
    }
    results.push(res);
  ...
  const summary = {
    ...
    failures: results.filter(
      (r) => !r.ok && r.message !== "PLACEHOLDER_SKIPPED",
    ).length,
    placeholders: results.filter((r) => r.message === "PLACEHOLDER_SKIPPED")
      .length,
  };
  console.log("\nSummary:", summary);
