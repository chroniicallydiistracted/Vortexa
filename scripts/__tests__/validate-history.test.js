...
describe("validate-history script", () => {
  it("passes on current history file", () => {
    const out = execSync("node scripts/validate-history.js", {
      cwd: root,
    }).toString();
  ...
  it("fails when required field removed", () => {
    const original = fs
      .readFileSync(path.join(root, "AGENTUPDATEHISTORY.jsonl"), "utf8")
      .split(/\n+/);
    const bad = [...original];
    ...
    for (let i = bad.length - 1; i > 0; i--) {
      try {
        const obj = JSON.parse(bad[i]);
        if (obj.change_type) {
          delete obj.change_type;
          bad[i] = JSON.stringify(obj);
          break;
        }
      } catch {
        /* skip non-json lines */
    ...
    let failed = false;
    try {
      execSync("node scripts/validate-history.js", {
        cwd: root,
        env: { ...process.env, HIST_FILE: tmpName },
      });
    } catch {
      failed = true;
    }
