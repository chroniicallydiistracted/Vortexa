...
if (!schemaObj || schemaObj.type !== "schema")
  fail('First line must have type="schema"');
...
for (let i = 1; i < lines.length; i++) {
  ...
  if (obj.actor && !enumActor.has(obj.actor))
    fail(`${ctx}: invalid actor ${obj.actor}`);
  ...
  if (obj.status && !enumStatus.has(obj.status))
    fail(`${ctx}: invalid status ${obj.status}`);
  ...
  if (Array.isArray(obj.changes)) {
    for (const ch of obj.changes) {
      ...
      if (
        ch.action &&
        !["add", "modify", "delete", "rename"].includes(ch.action)
      )
  ...
  if (Array.isArray(obj.paths)) {
    const changePaths = new Set(
      obj.changes.filter((c) => c.path).map((c) => c.path),
    );
    for (const p of obj.paths) {
      if (!changePaths.has(p))
        warn(`${ctx}: path '${p}' not found in changes[] list`);
    }
  ...
  if (obj.cost_impact && obj.cost_impact !== "none") {
    if (typeof obj.rationale === "string" && !/cost/i.test(obj.rationale))
      warn(
        `${ctx}: cost_impact ${obj.cost_impact} but rationale lacks 'cost' keyword`,
      );
  }
}
